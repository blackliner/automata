...
enum class MetaInfo : std::uint8_t { EMPTY, NOT_EMPTY };
...
class HashTable {
 ...
 public:
  ...
  void push(const T_Key& key, const T_Value& value) {
    if (item_count > load_factor * current_capacity) IncreaseSize();
    ...
    while (meta_storage[index] != MetaInfo::EMPTY)  // while there is already something
    {
      ...
      if (index == start_index) IncreaseSize();
    }
  ...
  std::vector<int>::iterator find(const T_Key& key) {
    ...
    while (meta_storage[index] != MetaInfo::EMPTY) {
      if (hash_storage[index] == hash_val) return std::next(value_storage.begin(), index);
